...
- 目前只包含了九宫格布局/九宫格拼音/qq86五笔的配置文件，后续会根据需求添加其他布局的配置文件。
...
#### 2026-07-30 (重大更新)
- **全面适配新版同文输入法**：修复主题样式错乱、按键错位等兼容性问题，优化视觉体验，保持极简设计理念。
- **新增九宫格布局预览**：清爽简洁的配色与精致的键位反馈，提升输入体验。
- **集成 Debug 完整包**：将配置、词库、样式文件统一打包，实现一键安装、开箱即用，免除手动配置的繁琐。
- **未来计划**：近期将正式发布完整 release 版本，欢迎试用并反馈问题。
